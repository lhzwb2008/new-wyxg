...
int main(int argc, char* argv[])
{
	...
	char *lyric; // 歌词：utf8每个汉字占3个char的位置，需自行去掉空格英文标点符号等非中文字符
	char **pinyin_list;         // 歌词对应的拼音，每个拼音最长占10个char  

	// 读取变量
	//if ( argc < 7 )
	//{
	//	printf("Need at least 6 parameters! current is %d.", argc - 1);
	//	return -1;
	//}
	...
	for ( i = 0; i < n; i++ )
	{ 
		strcpy(pinyin_list[i], "a");
		printf("%d. %s (len=%d %d)\n",i,pinyin_list[i],strlen(pinyin_list[i]), strlen(lyric)/n);
	}
